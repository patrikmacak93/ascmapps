...
NOBR_RE = re.compile(r"<nobr[^>]*>([\s\S]*?)</nobr>", re.I)
# Rozdeleni sloupcu ve druhem formatu (jeden radek = jeden <nobr>, sloupce
# oddelene 2+ mezerami).
...
# Typ zaznamu, kterym zacina kazdy datovy radek (druhy format nema oddelene
# bunky, takze datovy radek poznavame podle tohoto prefixu).
RECORD_TYPE = "ZPNZ_PR"

def _extract_rows_td(html):
"""
Format 1 (napr. export_pnz_sap.HTML): kazdy radek je <tr> a sloupce jsou
samostatne <td> bunky. Vraci (radky, saw_header), kde radek = seznam bunek.
"""
radky = []
saw_header = False
...
continue
radky.append(cells)
return radky, saw_header


def _extract_rows_nobr(html):
"""
Format 2 (napr. Job_PNZLISTSAP__Step_1.htm): SAP ALV export, kde je cely
radek v jednom <nobr> a sloupce jsou oddelene 2+ mezerami. Vraci
(radky, saw_header), kde radek = seznam sloupcu.

Datovy radek poznavame podle toho, ze zacina typem zaznamu (ZPNZ_PR).
Sloupec s PN muze mit vic hodnot oddelenych carkou; kdyz kolem carky byly
2+ mezery, rozpadne se na vic casti - proto vse od 3. sloupce dal spojime
zpet do jednoho retezce (spolecne zpracovani si pak vezme jen prvni PN).
"""
radky = []
saw_header = False
for nobr_match in NOBR_RE.finditer(html):
# Pozor: NEpouzivame cell_text (ten sloucil by vic mezer do jedne a
# sloupce by splynuly). Jen odstranime tagy a dekodujeme entity,
# mezery mezi sloupci zachovame.
text = decode_entities(TAG_RE.sub("", nobr_match.group(1)))
text = text.replace("\xa0", " ").strip() # nbsp -> mezera
if not text:
continue
if text.startswith(HEADER_FIRST_CELL): # hlavicka "Char20 CHAR35 ..."
saw_header = True
continue
if not text.startswith(RECORD_TYPE): # nadpis, cislo stranky, datum apod.
continue
parts = [p.strip() for p in COL_SPLIT_RE.split(text) if p.strip()]
if len(parts) < 3:
continue
# record_type, local_material, zbytek = PN (spojeny zpet)
radky.append([parts[0], parts[1], " ".join(parts[2:])])
return radky, saw_header
...
Kdyz je ve sloupci PN vic hodnot oddelenych carkou/strednikem (napr.
"PNE16020, PNZ5500"), zapise se jen PRVNI (PNE16020) a zbytek se zahodi.
Sloupce vystupu: (record_type, local_material, assigned_pn).

Podporuje DVA formaty SAP ALV exportu a rozpozna je automaticky:
1) sloupce v samostatnych <td> bunkach,
2) cely radek v jednom <nobr>, sloupce oddelene mezerami.
"""
warnings = []
rows = []

# Rozpoznani formatu: kdyz jsou v souboru <td> bunky, jde o format 1,
# jinak zkusime format 2 (<nobr> radky).
if "<td" in html.lower():
radky, saw_header = _extract_rows_td(html)
else:
radky, saw_header = _extract_rows_nobr(html)
